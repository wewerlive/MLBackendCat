...
from modelBackbone.model.attn_processor import SkipAttnProcessor
from modelBackbone.model.utils import get_trainable_module, init_adapter
from modelBackbone.utils import (compute_vae_encodings, numpy_to_pil, prepare_image,
...
class CatVTONPipeline:
    def __init__(
        ...
        # os.makedirs("combined_unet", exist_ok=True)
        # self.unet.save_pretrained("combined_unet")

        self.unet = UNet2DConditionModel.from_pretrained("roshanbiswa/twinverseCat").to(device, dtype=weight_dtype)
        # self.unet = UNet2DConditionModel.from_pretrained("combined_unet", subfolder="unet").to(device, dtype=weight_dtype)
        init_adapter(self.unet, cross_attn_cls=SkipAttnProcessor)  # Skip Cross-Attention

        # self.unet = quantize_dynamic(self.unet, {torch.nn.Linear, torch.nn.Conv2d}, dtype=torch.qint8)
        # Pytorch 2.0 Compile
        if compile:
